...
test('buildStickyProxyFromEnv requires links', () => {
    ...
    delete process.env.PROXY_LINKS;
    assert.throws(() => buildStickyProxyFromEnv(), /PROXY_URL|PROXY_LINKS/);
});
...
test('rotateStickySession refreshes session id in 711-style username', () => {
    const proxy = parseProxyLink('http://user-zone-custom-region-JP-session-12345678-sessTime-30:pass@h:10000');
